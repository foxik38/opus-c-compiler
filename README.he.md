...
קדם-מעבד, מנתח תחבירי, מייעל ומחולל קוד משלו — ותהליך הידור שאפשר לראות.
...
- 📺 **הידור שאפשר לראות** — כל שלב מדווח `OK` או `ERR`, זמן מעבד וזמן אמיתי, ומה הוא יצר.
...
- 🚀 **שתי רמות ייעול** — `-o none` לקוד שנכון באופן מובהק, ו-`-o prod` להקצאת אוגרים, מצבי
...
- ✨ **C23** — `constexpr`, `auto`, `typeof`, `nullptr`, `bool`, `#embed`, מאפיינים (attributes),
  `<stdckdint.h>`, מנייה עם טיפוס בסיס קבוע, מפרידי ספרות ועוד.
- 🧪 **הוכח על תוכניות אמיתיות** — SQLite, Lua, zlib, Duktape ואחרים נבנים עם occ ועוברים את
...
הקלט יכול להיות קובצי `.c` (מהודרים), `.s` (עוברים אסמבלר), וקובצי `.o` ו-`.a` (מקושרים).
...
כשמשהו משתבש, השלב שנכשל הופך ל-`ERR` ואחריו מגיעות הודעות האבחון — כל אחת עם המיקום, שורת
...
מובהק, והיא נקודת הייחוס שמולה נבדק `-o prod`. ‏`-o prod` שומר על השלד שלה ומסיר את העלויות:
...
הגרסאות מדפיסות את אותו סכום ביקורת). קצר יותר פירושו מהיר יותר.
...
סיור בתכונות נמצא ב-[`examples/c23_tour.c`](examples/c23_tour.c).
